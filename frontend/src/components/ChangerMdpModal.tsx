...
export default function ChangerMdpModal({ onSuccess }: Props) {
  ...
  const regles = [
    { label: '10 caractÃ¨res minimum', ok: form.nouveauMdp.length >= 10 },
    ...
    { label: 'Un caractÃ¨re spÃ©cial (@#$%&!)', ok: /[@#$%&!]/.test(form.nouveauMdp) },
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Mettre Ã  jour le flag en local
    ...
    <div className="modal-overlay">
      <div style={{ background: '#1a1d27', borderRadius: '10px', padding: '32px', maxWidth: '480px', width: '100%' }}>
        ...
        <p style={{ color: '#6b7280', fontSize: '14px', marginBottom: '24px' }}>
          Pour des raisons de sÃ©curitÃ©, vous devez dÃ©finir un nouveau mot de passe avant de continuer.
        </p>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ background: '#141720', border: '1px solid #2d3148', borderRadius: '8px', padding: '12px 16px', marginBottom: '16px' }}>
            <p style={{ fontSize: '12px', fontWeight: 600, color: '#6b7280', marginBottom: '8px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>RÃ¨gles de sÃ©curitÃ©</p>
            {regles.map(r => (
          ...
          <button type="submit" disabled={loading || !regles.every(r => r.ok)} className="btn btn-primary" style={{ width: '100%', justifyContent: 'center', opacity: regles.every(r => r.ok) ? 1 : 0.5 }}>
            {loading ? 'Enregistrement...' : 'DÃ©finir mon mot de passe'}
          </button>
        </form>
      </div>
    </div>
