...
export default function BaseAdmin({ titre, sousTitre, baseUrl, siteId, embedded }: Props) {
  ...
  async function handleCreate(e: React.FormEvent) {
    e.preventDefault()
    await post(`${baseUrl}/${siteId}/champs`, form)
    setForm(emptyChamp)
    reload()
  }

  async function handleUpdate(id: number) {
    await put(`${baseUrl}/champs/${id}`, editForm)
    setEditId(null)
    reload()
  }

  async function handleDelete(id: number) {
    await del(`${baseUrl}/champs/${id}`)
    setModal(null)
    reload()
  }
    ...
    <div>
      ...
      <div className="card">
        ...
        <table className="table" style={{ marginBottom: '20px' }}>
          <thead>
            <tr>
              <th>Ordre</th><th>Code</th><th>Label</th><th>Type</th><th>Obligatoire</th><th>Actif</th><th></th>
            </tr>
          </thead>
          <tbody>
            {champs.length === 0 && (
              ...
              <tr key={champ.id}>
                {editId === champ.id ? (
                  <>
                    ...
                    <td>
                      <div style={{ display: 'flex', gap: '6px' }}>
                        <button className="btn btn-primary btn-icon" onClick={() => handleUpdate(champ.id)}><Check size={14} /></button>
        ...
        <div style={{ borderTop: '1px solid #f1f5f9', paddingTop: '16px' }}>
          ...
          <form onSubmit={handleCreate} style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', alignItems: 'flex-end' }}>
            <div className="form-group" style={{ margin: 0 }}>
              ...
              <input required value={form.code} onChange={e => setForm(f => ({ ...f, code: e.target.value.toUpperCase().replace(/\s/g, '_') }))} className="form-input" placeholder="EX: NOM" style={{ width: '150px' }} />
            </div>
            <div className="form-group" style={{ margin: 0 }}>
              <label className="form-label">Label</label>
              <input required value={form.label} onChange={e => setForm(f => ({ ...f, label: e.target.value }))} className="form-input" placeholder="Ex: Nom" style={{ width: '150px' }} />
            </div>
            <div className="form-group" style={{ margin: 0 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px', paddingBottom: '2px' }}>
              ...
              <label htmlFor={`${baseUrl}-obligatoire`} style={{ fontSize: '13px', color: '#e2e8f0' }}>Obligatoire</label>
            </div>
